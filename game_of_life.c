...
int main(void) {
    ...
    noecho();
    curs_set(FALSE);
    timeout(0); // Устанавливаем неблокирующий ввод.
    keypad(stdscr, TRUE);

    while (1) { // Бесконечный цикл для продолжения игрового процесса.
        int ch = getch();
        if (ch == 'q') break; // Выход из цикла при нажатии 'q'.
        if (ch == 'f') delay = (delay > 50) ? delay - 50 : delay; // Ускорение игры при нажатии 'f'.
        if (ch == 's') delay = (delay < 2000) ? delay + 50 : delay; // Замедление игры при нажатии 's'.
